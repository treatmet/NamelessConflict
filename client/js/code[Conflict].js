...
socket.on('populateLoginPage', function(leaderboard, pcModeValue){
	...
	if (pcModeValue == 2){
		document.getElementById("titleText").innerHTML = "<a href='/'>GangWars</a>";
	}
	else {
		document.getElementById("titleText").innerHTML = "<a href='/'>RaceWar</a>";
	}
});
...
Img.weapon1Key.src = "/client/img/key-1.png";
...
function playNowClick(){
	if (cognitoUsername != ""){
		socket.emit('signIntoGameServer', cognitoSub, cognitoUsername);
	}
}
...
socket.on('authenticationSuccess', function(id, username){
	log('AUTH SUCCESS!');
	log("User " + username + " successfully authenticated with Cognito");
...
socket.on('authenticationFail', function(){
...
function drawBlocksOnBlockCanvas(){
	...
	//normalShadow();
	//block_canvas.clearRect(0,0,canvasWidth,canvasHeight); //Clears previous frame!!!!!!
		...
		if (Player.list[i].health > 0){
			if (Player.list[i].x * zoom + 47 * zoom + drawDistance > cameraX && Player.list[i].x * zoom - 47 * zoom - drawDistance < cameraX + canvasWidth && Player.list[i].y * zoom + 47 * zoom + drawDistance > cameraY && Player.list[i].y * zoom - 47 * zoom - drawDistance < cameraY + canvasHeight){
				...
				ctx.save();		
					...
					if (Player.list[i].cloak > 0.95){ctx.globalAlpha = 0.01;}
		...
		if (Player.list[i].x * zoom + 47 * zoom + drawDistance > cameraX && Player.list[i].x * zoom - 47 * zoom - drawDistance < cameraX + canvasWidth && Player.list[i].y * zoom + 47 * zoom + drawDistance > cameraY && Player.list[i].y * zoom - 47 * zoom - drawDistance < cameraY + canvasHeight){
			...
			ctx.save();			
				...
				if (Player.list[i].health > 0 && !(Player.list[i].cloakEngaged == true && Player.list[i].team != Player.list[myPlayer.id].team)){
					...
					//Replace this with above to use stroke instead of shadowBlur for names
					//ctx.strokeStyle="#FFFFFF";
					//ctx.lineWidth=0.5;
					//strokeAndFillText(Player.list[i].name,0, -Img.whitePlayerPistol.height/2 * zoom); //Draw myPlayer name above head, draw username, draw names			
				}			
...
//Version and debug text label1 //debug lable1
		...
		//ctx.fillText("cloakEngaged:" + Player.list[myPlayer.id].cloakEngaged, 5, 55); //debug
	}

/////////////////BOTTOM HUD//////////////////////////////	
	var liftBottomHUD = 10;
	largeCenterShadow();
	...
	if (!gameOver){
		if (myPlayer.DPAmmo > 0 || myPlayer.DPClip > 0){
			if (myPlayer.weapon == 2){
				drawImage(Img.weapon2Key, canvasWidth - 193, canvasHeight - 135 - liftBottomHUD);
				drawImage(Img.white, canvasWidth - 193 + Img.weapon2Key.width*(1/6), canvasHeight - 135 - liftBottomHUD + Img.weapon2Key.height+1, Img.weapon2Key.width*(2/3), 2);
			}
			else {
				drawImage(Img.weapon2Key, canvasWidth - 185, canvasHeight - 115 - liftBottomHUD, Img.weapon2Key.width*0.75, Img.weapon2Key.height*0.75);
			}
		}
		if (myPlayer.MGAmmo > 0 || myPlayer.MGClip > 0){
			if (myPlayer.weapon == 3){
				drawImage(Img.weapon3Key, canvasWidth - 133, canvasHeight - 135 - liftBottomHUD);
				drawImage(Img.white, canvasWidth - 133 + Img.weapon3Key.width*(1/6), canvasHeight - 135 - liftBottomHUD + Img.weapon3Key.height+1, Img.weapon3Key.width*(2/3), 2);
			}
			else {
				drawImage(Img.weapon3Key, canvasWidth - 125, canvasHeight - 115 - liftBottomHUD, Img.weapon3Key.width*0.75, Img.weapon3Key.height*0.75);
			}
		}
		if (myPlayer.SGAmmo > 0 || myPlayer.SGClip > 0){
			if (myPlayer.weapon == 4){
				drawImage(Img.weapon4Key, canvasWidth - 73, canvasHeight - 135 - liftBottomHUD);
				drawImage(Img.white, canvasWidth - 73 + Img.weapon4Key.width*(1/6), canvasHeight - 135 - liftBottomHUD + Img.weapon4Key.height+1, Img.weapon4Key.width*(2/3), 2);
			}
			else {
				drawImage(Img.weapon4Key, canvasWidth - 65, canvasHeight - 115 - liftBottomHUD, Img.weapon4Key.width*0.75, Img.weapon4Key.height*0.75);
			}
		}
	}
	...
	if (myPlayer.energy <= 25) {
		...
		drawImage(Img.energyRed, canvasWidth - Img.energyRed.width, canvasHeight - Img.energyRed.height);
		energyRedAlpha -= .1;
	}
	ctx.globalAlpha = 1;
	...
	strokeAndFillText(whiteScore,canvasWidth/2 - 70,(whiteScoreHeight + (whiteScoreHeight/2 + 16))/2);
	...
	strokeAndFillText(blackScore,canvasWidth/2 + 70,(blackScoreHeight + (blackScoreHeight/2 + 16))/2);
	...
	//log("gt:" + gametype + " min:" + minutesLeft + " sec:" + secondsLeft + " timeLimit:" + timeLimit.toString());
	...
	if (pregame == true && showStatOverlay == false){
		...
		if (minutesLeft == 0 && secondsLeft == 0 || gameOver == true){
			...
			ctx.font = '118px Electrolize';
			ctx.lineWidth=12;
			if (whiteScore > blackScore && gameOver == true){
			var whiteWinsText = "WHITES WIN";
				if (pcMode == 2){
					whiteWinsText = "RED WINS";
				}
				ctx.fillStyle="#FFFFFF";				
				strokeAndFillText(whiteWinsText,canvasWidth - 10,778);
			}
...
function drawUIElementsOnUICanvasElements(){
	UI_canvas.width = 0;
	UI_canvas.height = 0;
}
